...
export class MyApp {
    ...
    this.sqlite.create({
      ...
      .then((db: SQLiteObject) => {
       ...
       this.showAlert('db created');
        db.executeSql('create table if not exists w_category(cat_id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT(0),catogory VARCHAR(32))', {})
          .then(() => {
  ...
  insertData(db){
    db.sqlBatch(["INSERT INTO w_category(catogory) values('Status')",
      "INSERT INTO w_category(catogory) values('Shayari')",
      "INSERT INTO w_category(catogory) values('Jokes')",
      "INSERT INTO w_category(catogory) values('Quotes')",
      "INSERT INTO w_category(catogory) values('Group Links')"])
      .then(() => {
  ...
  selectQuery(db){
    ...
    if (data.rows.length > 0) {
      for (var i = 0; i < data.rows.length; i++) {
        cat.push({ category: data.rows.item(i).category});
